...
async def load_productid(message: types.Message, state : FSMContext):
    ...
    await store.next()
    await message.answer("верны ли данные?(да/нет)")
    ...
    if message.text.lower() == 'да':
        ...
        async with state.proxy() as data:
            await db_main.sql_insert_store_to_collection_products(
                productid=data['productid'],  # Передаем product_id
...
async def reg_handler_fsm_store(dp: Dispatcher):
          await dp.register_message_handler(cancel_fsm, Text(equals='Отмена', ignore_case=True),
                                state="*")
